...
public class CycleController {
    ...
    @GetMapping("/payment")
    public ResponseEntity<CartPaymentDto> payment() {
        List<CycleBasketEntity> cartCycles = basketRepository.findAll();
        int totalPayment = 0;

 

        for (CycleBasketEntity cartCycle : cartCycles) {
            totalPayment += (cartCycle.getCycle().getPrice() * cartCycle.getQuantity());
        }

 

        CartPaymentDto paymentDto = new CartPaymentDto();
        paymentDto.setCartCycles(cartCycles);
        paymentDto.setTotalPayment(totalPayment);
